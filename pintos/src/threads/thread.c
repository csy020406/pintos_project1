...
thread_unblock (struct thread *t) 
{
  ...
  list_insert_ordered (&ready_list, &t->elem, thread_compare_priority, NULL); // instead of list_push_back
...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread) 
    list_insert_ordered (&ready_list, &cur->elem, thread_compare_priority, NULL); // instead of list_push_back
...
advanced_calc_load_avg (void) //per 1 s
{
  ...
  if (thread_current() != idle_thread)
    ready = ready + 1;
  load_avg = add_fp (mult_fp (load_avg, div_fp(i_to_fp(59), i_to_fp(60))), mult_both (div_fp (i_to_fp(1), i_to_fp(60)), ready));
}
